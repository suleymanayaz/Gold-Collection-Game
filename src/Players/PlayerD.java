...
public class PlayerD extends Player  {
    ...
    public Point _bomp_Point_A;
    public Point _bomp_Point_B;
    public Point _bomp_Point_C;
    
    
    public PlayerD(Point _start){
        this._start = _start;
        this._move_Cost = 5;
        this._target_Cost = 20;
        ...
        this._bomp_Point_A = new Point();
        this._bomp_Point_B = new Point();
        this._bomp_Point_C = new Point();
    }

    
    
    
    public void _set_Target(Game _game,Point _target_Player_A,Point _current_Player_A,Point _target_Player_B,Point _current_Player_B,Point _target_Player_C,Point _current_Player_C, GameUI _game_UI){
        ...
        if(_distance(_start,_target_Player_A ) > _distance(_current_Player_A, _target_Player_A)){
            _bomp_Point_A = _target_Player_A;
        }
        if(_distance(_start,_target_Player_B  ) > _distance(_current_Player_B, _target_Player_B)){
            _bomp_Point_B = _target_Player_B;
        }
        if(_distance(_start,_target_Player_C ) > _distance(_current_Player_C, _target_Player_C)){
            _bomp_Point_C = _target_Player_C;
        }
        ...
        if(_have_Target && (_gold_Amount - _move_Cost) >= 0){
            ...
            for (int _rows = 0; _rows < _game.getLines(); _rows++){
                for(int _cols = 0; _cols < _game.getCols(); _cols++ ){
                    if(_game.getGrid()[_rows][_cols].isGold_Bool() || (_game.getGrid()[_rows][_cols].isHidden_Golden_Visible() && _game.getGrid()[_rows][_cols].isHidden_Golden_Bool())){
                        ...
                        if(!(_temp_Gold_Point.equals(_bomp_Point_A)) && !(_temp_Gold_Point.equals(_bomp_Point_B)) && !(_temp_Gold_Point.equals(_bomp_Point_C))){    
            ...
            if (_gold_Amount - _target_Cost < 0){
                _end = _start;
                System.out.println("Player D out of  Gold");
                _have_Gold_Bool = true;
            }
        }
    }
