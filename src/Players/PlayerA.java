...
public class PlayerA extends Player{
    ...
    public void _set_Target(Game _game){
        ...
        if(isHave_Target() && (getGold_Amount() - getMove_Cost()) >= 0){
            ...
            if(getEnd().x == 0 && getEnd().y == 0){
                setEnd(getStart());
                setHave_Gold_Bool(true);
            }
            if(!isHave_Gold_Bool()){
                setGold_Amount_Spent(getGold_Amount_Spent() + getTarget_Cost());
                setGold_Amount(getGold_Amount() - getTarget_Cost());
            }
        }else{
            if (getGold_Amount() - getTarget_Cost() < 0){
                setEnd(getStart());
    ...
    public void _find_Golden_Ways(Point _start, Point _end, Game _game){
        ...
        int _move_Number = getSteps_Number();
        _ways = new ArrayList<>();
        setClose_Neighbors(new ArrayList<>());
        setCurrent_Point(_start);
        setLast_Point(_end);
        ...
        if (_distance(getCurrent_Point(), getLast_Point()) <= getSetps_Number()){
            ...
            _move_Number = getSteps_Number();
            while(_move_Number != 0){
                setClose_Neighbors(_find_Neighbours(getCurrent_Point(), _move_Number, _game));
                for(Point _temp_Point : getClose_Neighbors()){
                    if(_rn.nextBoolean()){
                        if(!_ways.contains(_temp_Point)){
                            _ways.add(_temp_Point);
    ...
    public ArrayList<Point> _find_Neighbours (Point _current_Point,int _move_Number, Game _game){
       Point _up = null, _down = null, _left = null, _right =null;
        setFar_Neighbors(new ArrayList<>());
    ...
    public ArrayList<Point> _find_Neighbours_3 (Point _current_Point,int _move_Number, Game _game){
       ...
       if(getFar_Neighbors().size() >= 2){
           if( getFar_Neighbors().contains(_right)){
               if(_distance(_current_Point, getEnd()) < _distance(_right, getEnd()))
                   getFar_Neighbors().remove(_right);
           }
           if( getFar_Neighbors().contains(_left)){
               if(_distance(_current_Point, getEnd()) < _distance(_left, getEnd()))
                   getFar_Neighbors().remove(_left);
           }
           if( getFar_Neighbors().contains(_up)){
               if(_distance(_current_Point, getEnd()) < _distance(_up, getEnd()))
                   getFar_Neighbors().remove(_up);
           }
           if( getFar_Neighbors().contains(_down)){
               if(_distance(_current_Point, getEnd()) < _distance(_down, getEnd()))
                   getFar_Neighbors().remove(_down);
           }
